...
To include the latest release of the Cassandra plugins for **Akka 2.5.x** into your `sbt` project, add the following lines to your `build.sbt` file:
    ...
    libraryDependencies += Seq(
      "com.typesafe.akka" %% "akka-persistence-cassandra" % "0.86",
      "com.typesafe.akka" %% "akka-persistence-cassandra-launcher" % "0.86" % Test
    )
...
ScyllaDB is a wire compatible replacement for Cassandra. This project is not continuously tested against ScyllaDB. You can run the tests
...
0.80 introduces a completely different way to manage tags for events. You can skip right ahead to 0.86 without going to
0.80.
