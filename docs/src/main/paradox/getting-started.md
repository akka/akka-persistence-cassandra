...
Those versions are compatible with Cassandra 3.0.0 or higher, and it is also compatible with Cassandra 2.1.6 or higher (versions < 2.1.6 have a static column bug) if you configure `akka.persistence.cassandra.cassandra-2x-compat=on` in your `application.conf`.
