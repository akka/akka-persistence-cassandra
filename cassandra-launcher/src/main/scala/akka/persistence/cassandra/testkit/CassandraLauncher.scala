...
object CassandraLauncher {
  ...
  class CleanFailedException(message: String, cause: Throwable)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dir =
      if (args.length > 2) new File(args(2))
        ...
        new File(
          System.getProperty("CassandraLauncher.directory",
    ...
    if (cassandraDaemon.isEmpty) {
      ...
      if (!cassandraBundleFile.exists()) {
        val is = this.getClass.getClassLoader.getResourceAsStream(
          "akka/persistence/cassandra/launcher/cassandra-bundle.jar")
        try {
  ...
  private def prepareCassandraDirectory(cassandraDirectory: File,
                                        clean: Boolean): Unit = {
    if (clean) {
      try {
    ...
    if (!cassandraDirectory.exists)
      require(cassandraDirectory.mkdirs(),
              s"Couldn't create Cassandra directory [$cassandraDirectory]")
  }
  ...
  private def startForked(configFile: File,
    ...
    val classpathArgument = (classpath :+ cassandraBundle.getAbsolutePath)
      ...
      override def close(): Unit = {
        ...
        if (process.waitFor(ForcedShutdownTimeout.toMillis,
          ...
          if (exitStatus != 0 && exitStatus != 143) {
            sys.error(
              s"Cassandra exited with non zero status: ${process.exitValue()}")
          }
        } else {
          process.destroyForcibly()
          sys.error(
            s"Cassandra process did not stop within $ForcedShutdownTimeout, killing.")
        }
      }
    })
  }
      ...
      val retry = try {
        ...
        case ioe: IOException =>
          throw new RuntimeException(
            s"Cassandra did not start within $AwaitListenTimeout",
            ioe)
      }
      if (retry) tryConnect()
    }
