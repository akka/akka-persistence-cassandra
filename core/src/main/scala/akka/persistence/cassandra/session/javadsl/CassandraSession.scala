...
final class CassandraSession(
    delegate: akka.persistence.cassandra.session.scaladsl.CassandraSession) {
  ...
  def selectAll(stmt: String,
                bindValues: AnyRef*): CompletionStage[JList[Row]] =
  ...
  /**
    ...
    */
  @varargs
  def selectOne(stmt: String,
                bindValues: AnyRef*): CompletionStage[Optional[Row]] =
