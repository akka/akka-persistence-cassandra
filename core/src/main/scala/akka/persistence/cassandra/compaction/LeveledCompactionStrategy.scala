...
class LeveledCompactionStrategy(config: Config)
  ...
  require(ssTableSizeInMB > 0,
          s"sstable_size_in_mb must be larger than 0, but was $ssTableSizeInMB")
...
object LeveledCompactionStrategy
    extends CassandraCompactionStrategyConfig[LeveledCompactionStrategy] {
