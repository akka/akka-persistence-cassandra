 ...
 */
abstract class BaseCompactionStrategy(config: Config,
                                      className: String,
                                      propertyKeys: List[String])
    extends CassandraCompactionStrategy {
  require(config.hasPath("class") && config.getString("class") == className,
          s"Config does not specify a $className")
  require(
    config
      .entrySet()
      .asScala
  ...
  val tombstoneCompactionInterval: Long =
    if (config.hasPath("tombstone_compaction_interval"))
      config.getLong("tombstone_compaction_interval")
    else 86400
  val tombstoneThreshold: Double =
    if (config.hasPath("tombstone_threshold"))
      config.getDouble("tombstone_threshold")
    else 0.2
  val uncheckedTombstoneCompaction: Boolean =
    if (config.hasPath("unchecked_tombstone_compaction"))
  ...
  require(
    tombstoneThreshold > 0,
    s"tombstone_threshold must be greater than 0, but was $tombstoneThreshold")
...
object BaseCompactionStrategy
    extends CassandraCompactionStrategyConfig[BaseCompactionStrategy] {
