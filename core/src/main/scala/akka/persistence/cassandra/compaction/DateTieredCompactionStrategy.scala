...
class DateTieredCompactionStrategy(config: Config) extends BaseCompactionStrategy(config, DateTieredCompactionStrategy.ClassName, DateTieredCompactionStrategy.propertyKeys) {
  ...
  val timestampResolution: TimeUnit =
    if (config.hasPath("timestamp_resolution")) TimeUnit.valueOf(config.getString("timestamp_resolution"))
...
object DateTieredCompactionStrategy extends CassandraCompactionStrategyConfig[DateTieredCompactionStrategy] {
  ...
  override def propertyKeys: List[String] =
    (BaseCompactionStrategy.propertyKeys union List("base_time_seconds",
