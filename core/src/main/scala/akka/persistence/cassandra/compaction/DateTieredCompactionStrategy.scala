 ...
 */
@deprecated(
  "DateTieredCompaction is deprecated in Cassandra. Use TimeWindowCompaction.",
  "0.56")
class DateTieredCompactionStrategy(config: Config)
  ...
  val baseTimeSeconds: Long =
    if (config.hasPath("base_time_seconds")) config.getLong("base_time_seconds")
    else 3600
  val maxSSTableAgeDays: Int =
    if (config.hasPath("max_sstable_age_days"))
      config.getInt("max_sstable_age_days")
    else 365
  ...
  val timestampResolution: TimeUnit =
    if (config.hasPath("timestamp_resolution"))
      TimeUnit.valueOf(config.getString("timestamp_resolution"))
    else TimeUnit.MICROSECONDS

  require(baseTimeSeconds > 0,
          s"base_time_seconds must be greater than 0, but was $baseTimeSeconds")
  require(
    maxSSTableAgeDays >= 0,
    s"max_sstable_age_days must be larger than 0, but was $maxSSTableAgeDays")
...
object DateTieredCompactionStrategy
    extends CassandraCompactionStrategyConfig[DateTieredCompactionStrategy] {
  ...
  override def fromConfig(config: Config): DateTieredCompactionStrategy =
    new DateTieredCompactionStrategy(config)
}
