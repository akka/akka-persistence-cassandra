...
@InternalApi private[akka] class AllPersistenceIdsPublisher(
    ...
    extends QueryActorPublisher[String, AllPersistenceIdsState](refreshInterval,
                                                                config) {
  ...
  override protected def initialQuery(
      initialState: AllPersistenceIdsState): Future[Action] =
    query(initialState)

  override protected def completionCondition(
      state: AllPersistenceIdsState): Boolean = false
  ...
  override protected def updateState(
    ...
    if (state.knownPersistenceIds.contains(event)) {
      ...
      (Some(event),
       state.copy(knownPersistenceIds = state.knownPersistenceIds + event))
    }
  }

  override protected def requestNext(state: AllPersistenceIdsState,
