...
class CassandraReadJournal(system: ExtendedActorSystem, cfg: Config)
  ...
  private val eventsByPersistenceIdDeserializer
    : CassandraJournal.EventDeserializer =
  ...
  private def preparedSelectFromTagViewWithUpperBound
  ...
  /**
    * INTERNAL API
    */
  @InternalApi private[akka] def combinedEventsByPersistenceIdStmts
    : Future[CombinedEventsByPersistenceIdStmts] =
    for {
      ps1 <- preparedSelectEventsByPersistenceId
      ps2 <- preparedSelectDeletedTo
    } yield CombinedEventsByPersistenceIdStmts(ps1, ps2)

  @InternalApi private[akka] def combinedEventsByTagStmts
    : Future[CombinedEventsByTagStmts] =
    for {
  ...
  override def eventsByTag(tag: String,
    ...
    eventsByTagInternal(tag, offset)
      .mapConcat(r =>
        toEventEnvelope(r.persistentRepr, TimeBasedUUID(r.offset)))
      .mapMaterializedValue(_ => NotUsed)
      .named("eventsByTag-" + URLEncoder.encode(tag, ByteString.UTF_8))

  private[akka] def eventsByTagInternal(
      tag: String,
      offset: Offset): Source[UUIDPersistentRepr, NotUsed] =
    if (!config.eventsByTagEnabled)
      Source.failed(
        new IllegalStateException("Events by tag queries are disabled"))
    else {
      try {
        ...
        case NonFatal(e) =>
          ...
          log.debug("Could not run eventsByTag [{}] query, due to: {}",
                    tag,
                    e.getMessage)
          Source.failed(e)
      }
  ...
  private def deserializeEventsByTagRow
    : Flow[EventsByTagStage.UUIDRow, UUIDPersistentRepr, NotUsed] = {
    ...
    Flow[EventsByTagStage.UUIDRow]
      .mapAsync(queryPluginConfig.deserializationParallelism) { uuidRow =>
        val row = uuidRow.row
        eventsByTagDeserializer
          .deserializeEvent(row, deserializeEventAsync)
          .map { payload =>
            val repr = mapEvent(
              PersistentRepr(
                payload,
  ...
  private[akka] def scanTagSequenceNrs(tag: String, fromOffset: UUID)
  ...
  private[akka] def currentEventsByTagInternal(
      tag: String,
      offset: Offset): Source[UUIDPersistentRepr, NotUsed] =
    if (!config.eventsByTagEnabled)
      Source.failed(
      ...
      try {
        ...
        case NonFatal(e) =>
          // e.g. from cassandraSession, or selectStatement
          log.debug("Could not run currentEventsByTag [{}] query, due to: {}",
                    tag,
                    e.getMessage)
          Source.failed(e)
      }
  ...
  override def eventsByPersistenceId(
    ...
    eventsByPersistenceId(
      ...
      extractor = Extractors.persistentRepr(eventsByPersistenceIdDeserializer,
                                            serialization)
    ).mapMaterializedValue(_ => NotUsed)
  ...
  private[this] def toEventEnvelopes(
      ...
      offset: Long): immutable.Iterable[EventEnvelope] =
    adaptFromJournal(persistentRepr).map { payload =>
      EventEnvelope(Offset.sequence(offset),
  ...
  private[this] def toEventEnvelope(
      persistentRepr: PersistentRepr,
      offset: Offset): immutable.Iterable[EventEnvelope] =
    adaptFromJournal(persistentRepr).map { payload =>
  ...
  private def adaptFromJournal(
      persistentRepr: PersistentRepr): immutable.Iterable[Any] = {
  ...
  private[this] def persistenceIds(refreshInterval: Option[FiniteDuration],
      ...
      (s, ps) =>
        Source
          .actorPublisher[String](
            AllPersistenceIdsPublisher.props(refreshInterval,
