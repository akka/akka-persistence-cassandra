...
class CassandraJournal(cfg: Config)
  ...
  private lazy val deleteRetryPolicy = new LoggingRetryPolicy(
    new FixedRetryPolicy(config.deleteRetries))
  ...
  def preparedSelectDeletedTo =
    session
      ...
      .map(
        _.setConsistencyLevel(config.readConsistency)
          .setIdempotent(true)
  ...
  override def receivePluginInternal: Receive = {
    ...
    case DeleteFinished(persistenceId, toSequenceNr, result) =>
      log.debug("Delete finished for persistence id [{}] to [{}] result [{}]",
                persistenceId,
                toSequenceNr,
                result)
  ...
  private[akka] val writeRetryPolicy = new LoggingRetryPolicy(
    new FixedRetryPolicy(config.writeRetries))
  private[akka] val readRetryPolicy = new LoggingRetryPolicy(
    new FixedRetryPolicy(config.readRetries))
  ...
  override def asyncWriteMessages(
    ...
    def serialize(
        aw: Seq[(PersistentRepr, UUID)]): Future[SerializedAtomicWrite] = {
      ...
      serialized: Seq[SerializedAtomicWrite] =>
        val result: Future[Any] =
          if (messages.size <= config.maxMessageBatchSize) {
            ...
            def rec(todo: List[Seq[SerializedAtomicWrite]],
                    acc: List[Unit]): Future[List[Unit]] =
              todo match {
                case write :: remainder =>
                  writeMessages(write).flatMap(result =>
                    rec(remainder, result :: acc))
  ...
  private def extractTagWrites(
      serialized: Seq[SerializedAtomicWrite]): BulkTagWrite = {
    if (serialized.isEmpty) BulkTagWrite(Nil, Nil)
      ...
      val messagesByTag: Map[String, Seq[Serialized]] = serialized
        ...
        .groupBy(_._1)
        .map { case (tag, messages) => (tag, messages.map(_._2)) }
  ...
  private def writeMessages(
      atomicWrites: Seq[SerializedAtomicWrite]): Future[Unit] = {
  ...
  private def statementGroup(
      atomicWrites: Seq[SerializedAtomicWrite]): Seq[Future[BoundStatement]] = {
    ...
    if (config.eventsByTagEnabled) {
      ...
      highestSequenceNr.flatMap { seqNr =>
        if (seqNr == fromSequenceNr && seqNr != 0) {
          ...
          log.debug("New persistenceId [{}]. Sending blank tag progress.",
                    persistenceId)
          persistenceIdStarting(persistenceId,
                                Map.empty,
                                tagWrites.get,
                                persistentActor).map(_ => seqNr)
        } else {
          highestSequenceNr
        }
      }
  ...
  override def asyncDeleteMessagesTo(persistenceId: String,
    ...
    pendingDeletes.get(persistenceId) match {
      case null =>
        log.debug(
          "No outstanding delete for persistence id {}. Sequence nr: {}",
          persistenceId,
          toSequenceNr)
        // fast path, no outstanding deletes for this persistenceId
        val p = Promise[Unit]()
        pendingDeletes.put(persistenceId,
                           List(PendingDelete(persistenceId, toSequenceNr, p)))
      ...
      case otherDeletes =>
        ...
        if (otherDeletes.length > config.maxConcurrentDeletes) {
          ...
          pendingDeletes.put(
            persistenceId,
            otherDeletes :+ PendingDelete(persistenceId, toSequenceNr, p))
          p.future
        }
    }
  }

  private def delete(persistenceId: String,
                     toSequenceNr: Long): Future[Unit] = {

    def physicalDelete(lowestPartition: Long,
                       highestPartition: Long,
                       toSeqNr: Long): Future[Done] = {
      if (config.cassandra2xCompat) {
        def asyncDeleteMessages(partitionNr: Long,
                                messageIds: Seq[MessageId]): Future[Unit] = {
          val boundStatements = messageIds.map(
            mid =>
              preparedDeleteMessages.map(
                _.bind(mid.persistenceId,
                       partitionNr: JLong,
        ...
        val partitionInfos = (lowestPartition to highestPartition).map(
          partitionInfo(persistenceId, _, toSeqNr))
        val deleteResult =
          Future.sequence(partitionInfos.map(future =>
            future.flatMap(pi => {
              Future.sequence(
                (pi.minSequenceNr to pi.maxSequenceNr)
                  .grouped(config.maxMessageBatchSize)
                  .map {
                    group =>
                      {
                        val groupDeleteResult = asyncDeleteMessages(
                          pi.partitionNr,
                          group map (MessageId(persistenceId, _)))
                        groupDeleteResult.failed.foreach {
                          e =>
                            log.warning(
        ...
        val deleteResult =
          Future.sequence((lowestPartition to highestPartition).map {
            partitionNr =>
              val boundDeleteMessages = preparedDeleteMessages.map(
                _.bind(persistenceId, partitionNr: JLong, toSeqNr: JLong))
    ...
    /**
      ...
      */
    def logicalAndPhysicalDelete(highestDeletedSequenceNumber: Long,
                                 highestSequenceNr: Long): Future[Done] = {
      val lowestPartition = partitionNr(highestDeletedSequenceNumber + 1,
      ...
      logicalDelete.flatMap(_ =>
        physicalDelete(lowestPartition, highestPartition, toSeqNr))
    }

    val deleteResult = for {
      highestDeletedSequenceNumber <- asyncHighestDeletedSequenceNumber(
        persistenceId)
      highestSequenceNr <- {
      ...
      _ <- logicalAndPhysicalDelete(highestDeletedSequenceNumber,
                                    highestSequenceNr)
    } yield ()
  ...
  private def partitionInfo(persistenceId: String,
                            partitionNr: Long,
                            maxSequenceNr: Long): Future[PartitionInfo] = {
    val boundSelectHighestSequenceNr = preparedSelectHighestSequenceNr.map(
      _.bind(persistenceId, partitionNr: JLong))
      ...
      .map(
        row =>
          row
            ...
            .getOrElse(
              PartitionInfo(partitionNr, minSequenceNr(partitionNr), -1)))
  }

  private[akka] def asyncHighestDeletedSequenceNumber(
  ...
  private[akka] def asyncFindHighestSequenceNr(
    ...
    def find(currentPnr: Long, currentSnr: Long): Future[Long] = {
      ...
      val boundSelectHighestSequenceNr = preparedSelectHighestSequenceNr.map(
        _.bind(persistenceId, currentPnr: JLong))
      boundSelectHighestSequenceNr
        .flatMap(session.selectOne)
        .map { rowOption =>
          rowOption.map { row =>
  ...
  private def executeBatch(body: BatchStatement ⇒ Unit,
                           retryPolicy: RetryPolicy): Future[Unit] = {
  ...
  private def execute(stmt: Statement,
                      retryPolicy: RetryPolicy): Future[Unit] = {
...
@InternalApi private[akka] object CassandraJournal {
  ...
  private case class WriteFinished(pid: String, f: Future[Done])
      extends NoSerializationVerificationNeeded

  private case class DeleteFinished(pid: String,
                                    toSequenceNr: Long,
                                    f: Try[Unit])
      extends NoSerializationVerificationNeeded
  private case class PendingDelete(pid: String,
                                   toSequenceNr: Long,
                                   p: Promise[Unit])
      extends NoSerializationVerificationNeeded

  private case class SerializedAtomicWrite(persistenceId: String,
  ...
  private[akka] case class SerializedMeta(serialized: ByteBuffer,
                                          serManifest: String,
                                          serId: Int)

  private case class PartitionInfo(partitionNr: Long,
                                   minSequenceNr: Long,
                                   maxSequenceNr: Long)
  ...
  class EventDeserializer(system: ActorSystem) {
    ...
    private def hasColumn(column: String,
                          row: Row,
                          cached: Option[Boolean],
                          updateCache: Boolean => Unit): Boolean = {
      cached match {
        case Some(b) => b
        case None =>
    ...
    private val updateMetaColumnsCache: Boolean => Unit = b =>
    ...
    private val updateOldTagsColumnsCache: Boolean => Unit = b =>
      _hasOldTagsColumns = Some(b)
    ...
    private val updateTagsColumnCache: Boolean => Unit = b =>
      _hasTagsColumn = Some(b)
    def hasTagsColumn(row: Row): Boolean =
      hasColumn("tags", row, _hasTagsColumn, updateTagsColumnCache)

    def deserializeEvent(row: Row, async: Boolean)(
