...
@InternalApi private[akka] object TagWriter {
  ...
  private[akka] def props(settings: TagWriterSettings,
  ...
  private[akka] case class TagProgress(persistenceId: PersistenceId,
  ...
  case class PidProgress(seqNrFrom: SequenceNr,
                         seqNrTo: SequenceNr,
  ...
  final case class TagWriteDone(summary: TagWriteSummary,
                                doneNotify: Option[ActorRef])
      extends TagWriteFinished
  private final case class TagWriteFailed(
      reason: Throwable,
      failedEvents: Vector[(Serialized, TagPidSequenceNr)])
      extends TagWriteFinished
...
@InternalApi private[akka] class TagWriter(settings: TagWriterSettings,
                                           session: TagWritersSession,
                                           tag: String)
    extends Actor
    with Timers
  ...
  private def idle(
    ...
    case Flush =>
      if (buffer.nonEmpty) {
        ...
        write(buffer,
              Vector.empty[(Serialized, TagPidSequenceNr)],
              tagPidSequenceNrs,
              Some(sender()))
      } else {
    ...
    case twd: TagWriteDone =>
      log.error(
        "Received Done when in idle state. This is a bug. Please report with DEBUG logs: {}",
        twd)
    case ResetPersistenceId(_, tp @ TagProgress(pid, _, tagPidSequenceNr)) =>
      log.debug("Resetting pid {}. TagProgress {}", pid, tp)
      become(
        idle(buffer.filterNot(_._1.persistenceId == pid),
             tagPidSequenceNrs + (pid -> tagPidSequenceNr)))
  ...
  private def writeInProgress(
    ...
    case Flush =>
      log.debug(
        "External flush while write in progress. Will flush after write complete")
    ...
    case TagWriteDone(summary, doneNotify) =>
      ...
      summary.foreach {
        case (id, PidProgress(_, seqNrTo, tagPidSequenceNr, offset)) =>
          ...
          session
            .writeProgress(tag, id, seqNrTo, tagPidSequenceNr, offset)
            .onComplete {
              case Success(_) =>
              case Failure(t) =>
                log.warning(
      ...
      pubsub.foreach {
        _.mediator ! DistributedPubSubMediator.Publish(
    ...
    case ResetPersistenceId(_, tp @ TagProgress(pid, _, _)) =>
      ...
      become(
        writeInProgress(buffer.filterNot(_._1.persistenceId == pid),
                        tagPidSequenceNrs + (pid -> tp.pidTagSequenceNr))
      )
      sender() ! ResetPersistenceIdComplete
  }

  private def flushIfRequired(buffer: Vector[(Serialized, TagPidSequenceNr)],
                              tagSequenceNrs: Map[String, Long]): Unit =
    if (buffer.isEmpty) {
  ...
  ): (Map[String, TagPidSequenceNr], TagPidSequenceNr) = {
  ...
  /**
    ...
    */
  private def write(events: Vector[(Serialized, TagPidSequenceNr)],
    ...
    log.debug("Starting tag write of {} events. Summary: {}",
              events.size,
              writeSummary)
    val withFailure = session
      .writeBatch(tag, events)
  ...
  private def createTagWriteSummary(writes: Seq[(Serialized, TagPidSequenceNr)])
    : Map[PersistenceId, PidProgress] =
          ...
          if (event.sequenceNr <= to)
            throw new IllegalStateException(
              s"Expected events to be ordered by seqNr. ${event.persistenceId} " +
                s"Events: ${writes.map(e =>
                  (e._1.persistenceId, e._1.sequenceNr, e._1.timeUuid))}"
            )
          acc + (event.persistenceId -> PidProgress(from,
                                                    event.sequenceNr,
                                                    tagPidSequenceNr,
                                                    event.timeUuid))
        case None =>
          acc + (event.persistenceId -> PidProgress(event.sequenceNr,
                                                    event.sequenceNr,
                                                    tagPidSequenceNr,
                                                    event.timeUuid))
      }
  ...
  private def assignTagPidSequenceNumbers(
    ...
    events.foldLeft((currentTagPidSequenceNrs,
                     Vector.empty[(Serialized, TagPidSequenceNr)])) {
