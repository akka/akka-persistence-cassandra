...
trait CassandraTagRecovery {
  ...
  private[akka] def lookupTagProgress(
    ...
    preparedSelectTagProgressForPersistenceId
      ...
      .map(
        rs =>
          rs.all().asScala.foldLeft(Map.empty[String, TagProgress]) {
            (acc, row) =>
  ...
  private[akka] def tagScanningStartingSequenceNr(
      persistenceId: String): Future[SequenceNr] =
  ...
  private[akka] def sendMissingTagWriteRaw(
      tp: Map[Tag, TagProgress],
      to: ActorRef)(rawEvent: RawEvent): RawEvent = {
    // FIXME logging once stable
    log.debug("Processing tag write for event {} tags {}",
              rawEvent.sequenceNr,
              rawEvent.serialized.tags)
